document.addEventListener('DOMContentLoaded', () => {
  if (window.innerWidth < 1366){
    document.querySelector('.inner-cv').innerHTML = '<h4>Due to the current Coronavirus pandemic</h4> Will Call pickup available. <br> Masks or face shields are required. <br> Orders can be placed over the phone, via email and by texting our landline <br> (510) 215-5100 <br> Modified Hours of Operation: Monday thru Friday: 7AM-3PM. <a href="#" class="x"><h6 style="text-decoration: underline; margin: 5px;">dismiss</h6></a>'
...
function loadDoc3() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("covp-ajax").innerHTML =
      this.responseText;
    }
  };
  xhttp.open("GET", "covid.txt", true);
  xhttp.send();
}
